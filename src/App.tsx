import { ArrowRight, Banknote, Bell, Camera, CheckCircle2, Gift, MapPin, Recycle, ShieldCheck, Sparkles, Truck, Wallet } from 'lucide-react';
import { motion } from 'motion/react';

const categories = ['Paper', 'Plastic', 'Iron', 'E-waste'];
const steps = ['Upload scrap photo', 'AI estimates price', 'Confirm pickup', 'Earn payout'];

export default function App() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#f6f8fb] text-[#111827]">
      <nav className="fixed inset-x-0 top-0 z-50 border-b border-white/60 bg-white/75 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
          <div className="flex items-center gap-3 font-black tracking-tight"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#06b34f] text-white shadow-lg shadow-green-200"><Recycle /></span><span className="text-xl">Vangari <span className="text-[#06b34f]">ভাঙারি</span></span></div>
        ...
        <motion.div initial={{opacity:0,scale:.94}} animate={{opacity:1,scale:1}} transition={{duration:.7,delay:.15}} className="relative mx-auto w-full max-w-[410px]">
          ...
          <div className="relative rounded-[2.6rem] border border-white bg-white p-4 shadow-2xl shadow-gray-300">
            <div className="rounded-[2rem] bg-[#f6f8fb] p-6">
              ...
              <div className="mt-5 rounded-3xl border border-yellow-100 bg-yellow-50 p-4"><div className="flex items-center gap-4"><ShareIcon/><div><b>Invite and Earn 50 BDT</b><p className="text-sm text-gray-500">Send invite to your neighbor.</p></div></div></div>
              <h3 className="mt-7 text-2xl font-black">Scrap Categories</h3>
              <div className="mt-4 grid grid-cols-4 gap-3">{categories.map((c,i)=><div className="text-center text-xs font-bold text-gray-600" key={c}><div className="mb-2 rounded-2xl bg-white p-4 text-xl shadow-sm">{['Paper','Cup','Iron','PC'][i]}</div>{c}</div>)}</div>
            </div>
          </div>
        </motion.div>
      </section>

      <section id="how" className="bg-white py-20"><div className="mx-auto max-w-7xl px-5"><h2 className="text-4xl font-black tracking-tight md:text-5xl">Designed for the real Bangladeshi scrap market.</h2><div className="mt-10 grid gap-5 md:grid-cols-4">{steps.map((s,i)=><Card key={s} icon={[<Camera/>,<Sparkles/>,<Truck/>,<Wallet/>][i]} title={s} text={["User uploads a photo from home.","App gives an estimated value in BDT.","Collector arrives at the saved address.","User receives wallet balance and reward options."][i]}/>)}</div></div></section>
...
function Mini({icon,title,value}:{icon:React.ReactNode;title:string;value:string}){return <div className="rounded-3xl bg-white p-4 shadow-sm"><div className="mb-3 text-[#06b34f]">{icon}</div><p className="text-xs font-black uppercase tracking-wider text-gray-400">{title}</p><p className="text-2xl font-black">{value}</p></div>}
...
function Feature({icon,title,text}:{icon:React.ReactNode;title:string;text:string}){return <div className="rounded-[2rem] bg-white p-8 shadow-sm ring-1 ring-gray-100"><div className="mb-6 text-[#06b34f]">{icon}</div><h3 className="text-2xl font-black">{title}</h3><p className="mt-3 leading-7 text-gray-500">{text}</p></div>}
function ShareIcon(){return <div className="grid h-14 w-14 place-items-center rounded-2xl bg-yellow-100 text-yellow-600"><Gift/></div>}
